...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        self.img = ImageTk.PhotoImage(Image.open(f"{startdir}/dev/latest.jpg"))
    ...
    def say_hi(self):
        subprocess.call([f"bash {startdir}/bin/ph1280"], shell=True)
        self.img = ImageTk.PhotoImage(Image.open(f"{startdir}/dev/latest.jpg"))
        self.pic = Label(image=self.img)
        self.pic.grid(row=1, column=1, columnspan=3)
        print("click me")
    ...
    def middleclick(self):
        subprocess.call([f"bash {startdir}/dev/smallpic.sh"], shell=True)
        self.img = ImageTk.PhotoImage(Image.open(f"{startdir}/dev/second.jpg"))
        self.pic = Label(image=self.img)
        self.pic.grid(row=1, column=1, columnspan=3)
        print("click middle")
    ...
    def runpreview(self):
        subprocess.call(
            [f"{startdir}/py3/bin/python {startdir}/dev/preview.py"], shell=True
        )
